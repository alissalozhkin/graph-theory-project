...
def handle_left_click_station(subway: subway_system.Subway, buttons: pygame_buttons.Buttons,
    ...
    """Handle the given mouse click event, checking if the user left-clicked and selected a station.
...
def handle_right_click_station(screen: pygame.Surface, subway: subway_system.Subway,
    ...
    """Handle the given mouse click event, checking if the user right-clicked and selected a station
    to avoid.
    ...
    if event.button == 3 and buttons.get_button_colour('map view') == 'grey' and \
            screen.get_at((1113, 651)) != (139, 0, 0):
...
def handle_click_reset(screen: pygame.Surface, subway: subway_system.Subway,
    ...
    if event.button == 1 and buttons.get_button_colour('reset') == 'blue' and \
        ...
        # Draw a box that is the colour of the screen to cover
...
def handle_click_go(screen: pygame.Surface, subway: subway_system.Subway,
    ...
    if event.button == 1 and buttons.get_button_colour('go') == 'blue' and \
        ...
        if shortest_path == []:
            ...
            # Initiate and load sound
            pygame.mixer.init()
            path_sound = pygame.mixer.Sound('sounds/path.mp3')          # From zapsplat.com
            complete_sound = pygame.mixer.Sound('sounds/complete.mp3')  # From zapsplat.com
...
if __name__ == '__main__':
    ...
    python_ta.check_all(
        config={
            ...
            'extra-imports': ['python_ta.contracts', 'pygame', 'pygame_visualization',
                              'pygame_buttons', 'subway_system', 'plotly_visualization'],
